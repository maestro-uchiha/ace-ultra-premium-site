<# ============================================
   ...
   - Adds safe dark-theme init (no UI toggle, no CSS changes)
   ============================================ #>
...
# ---- inject a snippet before </body> (idempotent per marker) ----
function Inject-BottomScript([string]$html, [string]$snippet, [string]$markerName) {
  ...
  if ([regex]::IsMatch($html,$pat)) { return [regex]::Replace($html,$pat,$wrapped,1) }
...
# robots.txt (one canonical Sitemap line)
